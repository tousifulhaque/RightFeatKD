...
class ActTransformerAcc(nn.Module):
    def __init__(self, device, acc_frames=150, num_joints=29, in_chans=3, acc_coords=3, acc_embed=32, acc_features=18, adepth=4, num_heads=4, mlp_ratio=2., qkv_bias=True,
        ...
        acc_embed = acc_embed * num_joints
        ...
        if embed_type=='lin':
            self.Acc_coords_to_embedding = nn.Linear(acc_coords*num_joints, acc_embed) #Linear patch embedding
        else:
            self.Acc_coords_to_embedding = nn.Conv1d(acc_coords*num_joints, acc_embed, 1, 1) #Conv patch embedding
    ...
    def forward(self, inputs):
        ...
        sx = torch.reshape(sx, (b,-1,self.acc_coords) ) #B x Fa x 1 x 3
        #print(sx.shape)
        #print(x[:, :, 6, :self.joint_coords].shape)
        x[:, :, 6, :self.joint_coords] = (x[:, :, 6, :self.joint_coords] + sx) / 2
        
        #Get acceleration features
        sx = self.Acc_forward_features(x); #print("Input to ACC Transformer: ",sx) #in: F x Fa x 3 x 1,  op: B x St
